...
def load_audio(audiopath, sampling_rate):
    ...
    audio, lsr = torchaudio.load(audiopath)  # lsr = 22050

    # stereo to mono if needed
    if audio.size(0) != 1:
        audio = torch.mean(audio, dim=0, keepdim=True)

    if lsr != sampling_rate:  # sampling_rate  22050
...
class Xtts(BaseTTS):
    ...
    def get_gpt_cond_latents(self, audio, sr, length: int = 30, chunk_length: int = 6):
        ...
        """   # audio [1,308394] sr 22050 length 30 chunk_length 4
        if sr != 22050:
            audio = torchaudio.functional.resample(audio, sr, 22050)
        if length > 0:
            audio = audio[:, : 22050 * length]
        if self.args.gpt_use_perceiver_resampler: # yes
            style_embs = []
            for i in range(0, audio.shape[1], 22050 * chunk_length):
                audio_chunk = audio[:, i : i + 22050 * chunk_length]  # [1,88200]
                ...
                style_emb = self.gpt.get_style_emb(mel_chunk.to(self.device), None)  # [1,1024,32]
                style_embs.append(style_emb)

            # mean style embedding
            cond_latent = torch.stack(style_embs).mean(dim=0)  # len(style_embs)=4  [1,1024,32]
        else:
    ...
    def get_conditioning_latents(
        ...
        if not isinstance(audio_path, list):  # 'reference/dongdie/81-120_11.wav'
        ...
        for file_path in audio_paths:  # audio_paths ['reference/dongdie/81-120_11.wav']
            audio = load_audio(file_path, load_sr)
            audio = audio[:, : load_sr * max_ref_length].to(self.device)
            if sound_norm_refs:  # False
                audio = (audio / torch.abs(audio).max()) * 0.75
            if librosa_trim_db is not None:  # librosa_trim_db None
                audio = librosa.effects.trim(audio, top_db=librosa_trim_db)[0]  # trim silence
        ...
        )  # gpt_cond_latents [1, 32, 1024]

        if speaker_embeddings:
            speaker_embedding = torch.stack(speaker_embeddings)
            speaker_embedding = speaker_embedding.mean(dim=0)

        return gpt_cond_latents, speaker_embedding  # gpt_cond_latents [1,32,1024]  speaker_embedding [1,512,1]
    ...
    def full_inference(
        ...
        **hf_generate_kwargs,
        ...
        (gpt_cond_latent, speaker_embedding) = self.get_conditioning_latents(
            audio_path=ref_audio_path,  # 'reference/dongdie/81-120_11.wav'
            gpt_cond_len=gpt_cond_len,  # 30
            gpt_cond_chunk_len=gpt_cond_chunk_len, # 4
            max_ref_length=max_ref_len,  # 30
            sound_norm_refs=sound_norm_refs,  # False
        )  # gpt_cond_latent [1,32,1024]  speaker_embedding [1,512,1]
    ...
    def inference(
        ...
        **hf_generate_kwargs,
        ...
        gpt_latents_list = []
        for sent in text:
            sent = sent.strip().lower()  # '[zh-cn]ji3ke1da4er2liang4de5xing1xing1gua4zai4ye4kong1[SPACE]fang3fu2shi4tian1shang4de5ren2er2ti2zhe5deng1long2zai4xun2shi4na4hao4han4de5tai4kong1'
            ...
            with torch.no_grad():
                ...
                )  # self.gpt.code_stride_len = 1024   expected_output_len = 149*1024 = 152576

                text_len = torch.tensor([text_tokens.shape[-1]], device=self.device)  # text_len = 55
                gpt_latents = self.gpt(
                    text_tokens,  # [1,55]
                    text_len,  # 55
                    gpt_codes,  # [1,149]
                    expected_output_len,  # 149*1024
                    cond_latents=gpt_cond_latent,  # [1,32,1024]
                    return_attentions=False,
                    return_latent=True,
                )  # gpt_latents [1,149,1024]

                if length_scale != 1.0:  # False
                    gpt_latents = F.interpolate(
                        gpt_latents.transpose(1, 2), scale_factor=length_scale, mode="linear"
                    ).transpose(1, 2)

                gpt_latents_list.append(gpt_latents.cpu())
                wavs.append(self.hifigan_decoder(gpt_latents, g=speaker_embedding).cpu().squeeze())   # [1,149,1024]  [1,512,1]
